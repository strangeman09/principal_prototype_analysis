...
lin_model.load_state_dict(torch.load('piecwise_prot_indata.pth'))
...
for _ in range(NUM_ITERATIONS):
    ...
    with open('/export/kbodla/car_racing/X_train_feat.pkl', 'rb') as f:
        X_train = pickle.load(f)
    with open('/export/kbodla/car_racing/a_train.pkl', 'rb') as f:
        real_actions = pickle.load(f)

    X_train = np.vstack(X_train) 
    real_actions = np.vstack(real_actions)
    ...
    # tensor_x = tensor_x.reshape(30386,256)
    # tensor_y = tensor_y.reshape(30386,3)
    ...
    # nn_human_x = np.load("prot_replace_proxyanchormoment_indata.npy")
    nn_human_x = np.load('piecewise_prot_indata.npy')
    print(nn_human_x.shape)
    # nn_human_x = np.load("prot_replace_proxyanchormoment_intrain.npy")

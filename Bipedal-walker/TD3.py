...
class Actor(nn.Module):
    ...
    def forward(self, state):
        ...
        aa = F.relu(self.l2(a))
        a = torch.tanh(self.l3(aa)) * self.max_action
        return a, aa
...
class TD3:
    ...
    def select_action(self, state):
        ...
        return self.actor(state)[0].cpu().data.numpy().flatten(), self.actor(state)[1].cpu().data.numpy().flatten() ## state added
    ...
    def load(self, directory, name):
        ...
        self.actor_target.load_state_dict(torch.load(f"{directory}/{name}_actor_target.pth", map_location=device))
    
        self.critic_1.load_state_dict(torch.load(f"{directory}/{name}_crtic_1.pth", map_location=device))
        self.critic_1_target.load_state_dict(torch.load(f"{directory}/{name}_critic_1_target.pth", map_location=device))
    
        self.critic_2.load_state_dict(torch.load(f"{directory}/{name}_crtic_2.pth", map_location=device))
        self.critic_2_target.load_state_dict(torch.load(f"{directory}/{name}_critic_2_target.pth", map_location=device))
    ...
    def load_actor(self, directory, name):
        ...
        self.actor_target.load_state_dict(torch.load('%s/%s_actor_target.pth' % (directory, name), map_location='cpu'))
